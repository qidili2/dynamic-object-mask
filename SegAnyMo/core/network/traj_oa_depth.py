...
# 使用sam2对每一帧进行掩码分割
def load_sam2_instance_masks(mask_dir: str, frame_names: list):
    """
    Loads per-frame SAM2 instance masks into a tensor.
    mask_dir: Path to the saved instance masks (*.png)
    frame_names: List of frame names (without extension)
    Returns: [1, L, H, W] LongTensor
    """
    masks = []
    for name in frame_names:
        ...
        mask = cv2.imread(path, cv2.IMREAD_UNCHANGED)  # load grayscale image
        masks.append(mask)

    masks = np.stack(masks, axis=0)  # [L, H, W]
    masks = torch.from_numpy(masks).unsqueeze(0).long()  # [1, L, H, W]
    return masks
...
class traj_oa_depth(nn.Module):
    ...
    def forward(self, batch):
        ...
        enc_end = time.time()
        print(f"[Timing] Encoder time: {enc_end - enc_start:.2f} seconds")
        ...
        dec_end = time.time()
        print(f"[Timing] Decoder time: {dec_end - dec_start:.2f} seconds")
        if torch.isnan(mask).any() or torch.isinf(mask).any():

...
class DAVISEvaluation(object):
    ...
    def evaluate(self, res_path, metric=('J', 'F'), debug=True):
        import sys
        ...
        seq_list = list(self.dataset.get_sequences())
        print("[DEBUG] DAVIS evaluate(): seq_list =", seq_list)

        for seq in tqdm(seq_list):
            # 进入循环的一定会打印这行
            print(f"[DEBUG] === Evaluating seq '{seq}' ===")
            try:
                print(f"[DEBUG] {seq}: calling get_all_masks(...)")
                all_gt_masks, all_void_masks, all_masks_id = self.dataset.get_all_masks(seq, True)
                print(f"[DEBUG] {seq}: got GT masks: "
                    f"all_gt_masks.shape={all_gt_masks.shape}, "
                    f"all_void_masks.shape={None if all_void_masks is None else all_void_masks.shape}, "
                    f"all_masks_id={all_masks_id}")

                if self.task == 'semi-supervised':
                    all_gt_masks, all_masks_id = all_gt_masks[:, 1:-1, :, :], all_masks_id[1:-1]
                    print(f"[DEBUG] {seq}: after semi-supervised slicing, "
                        f"all_gt_masks.shape={all_gt_masks.shape}, all_masks_id={all_masks_id}")

                print(f"[DEBUG] {seq}: calling results.read_masks(...) in {res_path}")
                all_res_masks = results.read_masks(seq, all_masks_id)
                if all_res_masks is None:
                    print(f"[WARN] {seq}: results.read_masks returned None, skip this seq.")
                    continue
                print(f"[DEBUG] {seq}: got RES masks: all_res_masks.shape={all_res_masks.shape}")

                if self.task == 'unsupervised':
                    print(f"[DEBUG] {seq}: calling _evaluate_unsupervised(...)")
                    ...
                    print(f"[DEBUG] {seq}: calling _evaluate_semisupervised(...)")
                    j_metrics_res, f_metrics_res = self._evaluate_semisupervised(
                        all_gt_masks, all_res_masks, None, metric
                    )
                print(f"[DEBUG] {seq}: j_metrics_res.shape={j_metrics_res.shape}, "
                    f"f_metrics_res.shape={f_metrics_res.shape}")
                ...
                print(f"[DEBUG] {seq}: finished OK.")

            except Exception as e:
                import traceback
                print(f"[ERROR] Exception when evaluating seq '{seq}': {repr(e)}")
                traceback.print_exc()
                # 不想整个 eval 掉的话可以改成 continue
                raise

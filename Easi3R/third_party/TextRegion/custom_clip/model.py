...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_value=False, csa=True, return_all=True, with_attn=False,
                return_block=-1, return_last_attn_output=False):
        ...
        for blk in self.transformer.resblocks[return_block:]:
            if return_value:
                return x, blk
            if with_attn:
                x, attn_weights = self.custom_attn(blk.attn, blk.ln_1(x), csa=csa, with_attn=with_attn)
            else:
                x = x + self.custom_attn(blk.attn, blk.ln_1(x), csa=csa, with_attn=with_attn)
            x = x + blk.mlp(blk.ln_2(x))

        if return_last_attn_output:
            return x

        x = x.permute(1, 0, 2)  # LND -> NLD
        if return_all:
            output = self.ln_post(x) @ self.proj
            if with_attn:
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_value=False, return_all=False, csa=False, with_attn=False, return_block=-1, return_last_attn_output=False):
        return self.visual(image.type(self.dtype), return_value=return_value, return_all=return_all, csa=csa, with_attn=with_attn,
                           return_block=return_block, return_last_attn_output=return_last_attn_output)

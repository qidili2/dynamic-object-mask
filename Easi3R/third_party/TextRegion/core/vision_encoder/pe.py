...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        attn_mask: Optional[torch.Tensor] = None,
        layer_idx: int = -1,
        attend_layer_idx: int = -1,
        region_attn_mask: Optional[torch.Tensor] = None,
    ):
        stop_idx = (self.layers + layer_idx) % self.layers
        start_attend_idx = (self.layers + attend_layer_idx) % self.layers
        ...
        for i, r in enumerate(self.resblocks):
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                # Modified by @Ava
                if i < start_attend_idx:
                    x = r(x, attn_mask=attn_mask)
                else:
                    if region_attn_mask is not None:
                        if len(x) != len(region_attn_mask):
                            x = x.repeat(region_attn_mask.shape[0], 1, 1)

                        x = r(x, attn_mask=region_attn_mask)
                    else:
                        x = r(x, attn_mask=attn_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(
        ...
        x = self.ln_pre(x)
        x = self.transformer(x, layer_idx=layer_idx, attn_mask=attn_mask, attend_layer_idx=attend_layer_idx, region_attn_mask=region_attn_mask)
    ...
    def forward(self, x: torch.Tensor, return_value: bool = False, attn_mask: torch.bool = None, attend_layer_idx: int = -1, region_attn_mask: Optional[torch.Tensor] = None, **kwargs):
        x = self.forward_features(x, norm=True, attn_mask=attn_mask, attend_layer_idx=attend_layer_idx, region_attn_mask=region_attn_mask, **kwargs)

        if return_value:
            return x, self.attn_pool
...
class CLIP(TextTransformer):
    ...
    def encode_image(self, image, normalize: bool = False, return_value: bool = False, region_attn_mask: torch.float = None, attend_layer_idx: int = -1):
        x = self.visual(image, return_value=return_value, region_attn_mask=region_attn_mask, attend_layer_idx=attend_layer_idx)

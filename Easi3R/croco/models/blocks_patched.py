...
def forward(self, query, key, value, qpos, kpos, qmask=None, kmask=None,
            ...
            return_bg_means=False, return_qk=False,):
        B, Nq, C = query.shape
        Nk = key.shape[1]
        Nv = value.shape[1]
        ...
        def _make_delta_map(x, mean_delta, bg_tok, ref_tok, ref_mask):
            """Build per-token delta:
            - default: mean_delta broadcast to all tokens
            - override at positions where (bg_tok & ref_mask) is True with (x - ref_tok)
            Assumes B==1 (same as _bg_mean)."""
            if mean_delta is None:
                return None
            # mean_delta: [1, heads, 1, dh]
            delta_full = mean_delta.expand(1, mean_delta.shape[1], x.shape[2], mean_delta.shape[3]).clone()
            if (bg_tok is None) or (ref_tok is None) or (ref_mask is None):
                return delta_full
            if x.shape[0] != 1:
                return delta_full
            sel_a = (bg_tok.squeeze(-1) > 0)       # [B,N]
            sel_b = (ref_mask.squeeze(-1) > 0)     # [B,N]
            if (sel_a.shape != sel_b.shape) or (ref_tok.shape != x.shape):
                return delta_full
            inter = (sel_a & sel_b)                # [B,N]
            if inter[0].sum() == 0:
                return delta_full
            delta_tok = (x - ref_tok)              # [B,heads,N,dh]
            delta_full[0, :, inter[0], :] = delta_tok[0, :, inter[0], :]
            return delta_full

        if align_q and (q_bg_mean is not None) and (q_bg_ref_mean is not None):
            mean_delta = (q_bg_mean - q_bg_ref_mean)  # [1, heads, 1, dh]
            delta_map = _make_delta_map(q, mean_delta, q_bg_tok, q_bg_ref_tok, q_bg_ref_tok_mask)
            q[0] = q[0] - delta_map[0]

        if align_k and (k_bg_mean is not None) and (k_bg_ref_mean is not None):
            mean_delta = (k_bg_mean - k_bg_ref_mean)  # [1, heads, 1, dh]
            delta_map = _make_delta_map(k, mean_delta, k_bg_tok, k_bg_ref_tok, k_bg_ref_tok_mask)
            k[0] = k[0] - delta_map[0]
        # ----------------- LIMITED DEBUG PRINT -----------------
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        if return_bg_means and return_qk:
            return x, attn_before_softmax, q_bg_mean, k_bg_mean, q, k
        if return_bg_means:
            return x, attn_before_softmax, q_bg_mean, k_bg_mean
        if return_qk:
            return x, attn_before_softmax, q, k
        return x, attn_before_softmax
...
def forward(self, x, y, xpos, ypos, xmask, ymask, self_mask,
        ...
        return_bg_means=return_bg_means, return_qk=return_qk,
        )

        q_tok = k_tok = None
        if return_bg_means and return_qk:
            cross_attn_output, cross_attn, q_bg_mean, k_bg_mean, q_tok, k_tok = out
        elif return_bg_means:
            cross_attn_output, cross_attn, q_bg_mean, k_bg_mean = out
        elif return_qk:
            cross_attn_output, cross_attn, q_tok, k_tok = out
        ...
        if return_bg_means and return_qk:
            return x, y, self_attn, cross_attn, q_bg_mean, k_bg_mean, q_tok, k_tok
        if return_bg_means:
            return x, y, self_attn, cross_attn, q_bg_mean, k_bg_mean
        if return_qk:
            return x, y, self_attn, cross_attn, q_tok, k_tok
